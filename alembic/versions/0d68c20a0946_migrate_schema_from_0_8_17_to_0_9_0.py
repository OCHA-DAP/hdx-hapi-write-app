...
def upgrade() -> None:
    ...
    # We can't create the new primary keys yet because we'll get conflicts with the current data
    # But we need to drop the old primary keys so that we don't get conflicts with the new data
    ...
    # Add value to Enum
    op.execute("ALTER TYPE pricetype ADD VALUE 'Producer'")
...
def downgrade() -> None:
    ...
    old_status_enum = postgresql.ENUM('Farm Gate', 'Retail', 'Wholesale', name='pricetype_new')
    old_status_enum.create(op.get_bind())
    op.execute("""
        ALTER TABLE food_price_vat 
        ALTER COLUMN price_type TYPE pricetype_new USING price_type::text::pricetype_new
    """)
    op.execute('DROP TYPE pricetype')
    op.execute("ALTER TYPE pricetype_new RENAME TO pricetype")
